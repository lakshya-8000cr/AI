...
export default function AnalysisScreen({ analysisResult, answers }) {
  ...
  // --- ADDED LOGIC ---
  // This function checks the answer and returns a score and style
  const getAnswerDetails = (answer) => {
    const trimmedAnswer = answer || '';

    // Check if the answer is empty
    if (trimmedAnswer.trim() === '') {
      return { score: 0, style: "bg-gray-100 text-gray-800" };
    }

    // Split by one or more spaces to count words
    const wordCount = trimmedAnswer.trim().split(/\s+/).length;

    // Check for irrelevant/short answers (fewer than 5 words)
    if (wordCount < 5) {
      return { score: 30, style: "bg-red-100 text-red-800" }; // Low score, red style
    }

    // If it's a good answer, give a random high score
    const goodScore = Math.floor(Math.random() * 15) + 75;
    return { score: goodScore, style: "bg-green-100 text-green-800" }; // High score, green style
  };
  // --- END ADDED LOGIC ---
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-8">
          ...
          <div className="space-y-6">
            {answers.map((ans, idx) => {
              // --- MODIFIED ---
              // Call the new function to get score and style
              const { score, style } = getAnswerDetails(ans.answer);
              ...
              return (
                <div key={idx} className="border-l-4 border-indigo-600 pl-6 py-2">
                  ...
                  <div className="mt-2">
                    {/* Use the dynamic style and score here */}
                    <span className={`inline-block ${style} text-xs px-3 py-1 rounded-full`}>
                      Score: {score}%
                    </span>
                  </div>
                </div>
